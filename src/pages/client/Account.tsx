...
const LOYALTY_ITEMS = [
  { label: "Mes avantages", icon: Gift, path: "/client/advantages", badge: "pts" },
...
export default function AccountPage() {
  ...
  if (loading) return <div className="p-10 text-center text-foodiz-gray">Chargement de votre profil...</div>;
    ...
    <div className="animate-fade-in-up space-y-6">
      ...
      <div className="foodiz-card p-5 flex items-center gap-4">
        <div className="w-14 h-14 rounded-full bg-foodiz-gradient-gold border-2 border-foodiz-gold/30 flex items-center justify-center">
          <span className="text-foodiz-gold font-serif font-bold text-xl">{profile?.full_name?.charAt(0).toUpperCase() || 'U'}</span>
        </div>
        <div className="flex-1">
          <h2 className="foodiz-title text-lg">{profile?.full_name || 'Utilisateur'}</h2>
          <p className="text-foodiz-gray text-xs">{profile?.email}</p>
          <div className="flex items-center gap-2 mt-1.5">
            <GoldIcon icon={Star} size={12} />
            <span className="text-foodiz-gold text-xs font-semibold">{profile?.points || 0} points Foodiz</span>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-1">
        ...
        {LOYALTY_ITEMS.map((item) => (
          ...
          >
            ...
            {item.badge === "pts" && (profile?.points || 0) > 0 && (
      ...
      {/* Logout */}
      <button onClick={handleLogout} className="w-full flex items-center justify-center gap-2 py-3 text-foodiz-gray hover:text-foodiz-cream transition-colors text-sm">
        <LogOut size={16} />
        Se déconnecter
      </button>
    </div>
