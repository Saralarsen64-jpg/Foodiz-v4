...
export default function AccountPage() {
  ...
  return (
    <div className="animate-fade-in-up space-y-6">
      ...
      <div className="relative overflow-hidden rounded-[2rem] border border-foodiz-gold/20 bg-[linear-gradient(135deg,rgba(216,168,79,0.16),rgba(17,17,17,0.95)_28%,rgba(5,5,5,1)_100%)] p-5 shadow-[0_18px_60px_rgba(0,0,0,0.45),0_0_28px_rgba(216,168,79,0.06)]">
        ...
        <div className="relative z-10 flex items-center gap-4">
          <div className="w-16 h-16 rounded-full bg-foodiz-gradient-gold border-2 border-foodiz-gold/30 flex items-center justify-center shadow-[0_12px_30px_rgba(216,168,79,0.18)] shrink-0">
            <span className="text-foodiz-black font-serif font-bold text-2xl">A</span>
          </div>
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap mb-1">
              <span className="px-2.5 py-1 rounded-full bg-black/30 border border-foodiz-gold/15 text-[9px] uppercase tracking-[0.18em] text-foodiz-gold font-bold">
                Foodiz Club
              </span>
              <span className="px-2.5 py-1 rounded-full bg-foodiz-gold/10 border border-foodiz-gold/20 text-[9px] uppercase tracking-[0.18em] text-foodiz-gold font-bold">
                Membre Signature
              </span>
            </div>
            <h2 className="foodiz-title text-xl">Alexandre</h2>
            <p className="text-foodiz-gray text-xs">alexandre@email.com</p>
          </div>
          <button
            onClick={() => navigate("/client/account/personal-info")}
            className="text-foodiz-gold text-xs border border-foodiz-gold/20 rounded-full px-3 py-1.5 hover:border-foodiz-gold/40 transition-all"
          >
            Modifier
          </button>
        </div>
        ...
        <div className="relative z-10 mt-5 grid grid-cols-2 gap-3">
          <div className="rounded-[1.2rem] bg-white/[0.03] border border-foodiz-gold/10 p-4">
            ...
            <div className="flex items-center gap-2 mt-2">
              <GoldIcon icon={Star} size={14} />
              <span className="text-foodiz-gold text-2xl font-bold font-serif">1 240</span>
            </div>
          </div>
          <div className="rounded-[1.2rem] bg-white/[0.03] border border-foodiz-gold/10 p-4">
            <p className="text-[10px] text-foodiz-gray uppercase tracking-[0.18em] font-bold">Avantage actif</p>
            <div className="flex items-center gap-2 mt-2">
              <GoldIcon icon={Sparkles} size={14} />
              <span className="text-foodiz-cream text-sm font-medium">Aucun verrouillage</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <button
        onClick={() => navigate("/client/account/referral")}
        className="w-full overflow-hidden rounded-[1.6rem] border border-foodiz-gold/15 bg-[linear-gradient(135deg,rgba(216,168,79,0.08),rgba(17,17,17,0.95)_26%,rgba(10,10,10,1)_100%)] p-5 text-left shadow-[0_14px_40px_rgba(0,0,0,0.3)]"
      >
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-[10px] text-foodiz-gold uppercase tracking-[0.2em] font-bold mb-2">Parrainage Foodiz</p>
            <h3 className="foodiz-title text-lg mb-2">Invitez vos proches</h3>
            <p className="text-foodiz-gray text-sm leading-relaxed max-w-[260px]">
              Gagnez 500 points Foodiz pour chaque ami invité ayant validé sa première commande.
            </p>
          </div>
      ...
      <div className="space-y-2">
        <div className="flex items-center gap-2 px-1">
          <ShieldCheck size={14} className="text-foodiz-gold" />
          <h3 className="text-[10px] font-semibold text-foodiz-gray uppercase tracking-widest">
            Fidélité & Avantages
          </h3>
        </div>
        {LOYALTY_ITEMS.map((item) => (
          <button
            ...
            className="w-full foodiz-card p-4 flex items-center gap-3 text-left hover:border-foodiz-gold/30 transition-all bg-[linear-gradient(145deg,rgba(216,168,79,0.05),rgba(17,17,17,0.98)_25%,rgba(10,10,10,1)_100%)]"
          >
            ...
            {item.badge && (
              <span className="text-[10px] text-foodiz-gold bg-foodiz-gold/10 px-2 py-1 rounded-full font-semibold border border-foodiz-gold/15">
                {item.badge}
              </span>
            )}
      ...
      <div className="space-y-2">
        <div className="flex items-center gap-2 px-1">
          <User size={14} className="text-foodiz-gold" />
          <h3 className="text-[10px] font-semibold text-foodiz-gray uppercase tracking-widest">
            Compte
          </h3>
        </div>
        {MENU_ITEMS.map((item) => (
          ...
          >
            <div className="w-10 h-10 rounded-xl bg-white/[0.03] border border-foodiz-gold/10 flex items-center justify-center shrink-0">
              <GoldIcon icon={item.icon} size={18} />
            </div>
      ...
      <div className="space-y-2">
        <div className="flex items-center gap-2 px-1">
          <HelpCircle size={14} className="text-foodiz-gold" />
          <h3 className="text-[10px] font-semibold text-foodiz-gray uppercase tracking-widest">
            Assistance
          </h3>
        </div>
      ...
      {/* Logout */}
      <button className="w-full flex items-center justify-center gap-2 py-3 text-foodiz-gray hover:text-foodiz-cream transition-colors text-sm">
        <LogOut size={16} />
        Se déconnecter
      </button>
    </div>
