...
export default function CheckoutPage() {
  ...
  // Récupérer le vrai UUID du restaurant (Simulation pour l'exemple, à lier dynamiquement plus tard)
  useEffect(() => {
    const fetchRestaurant = async () => {
      // Pour l'instant, on cherche le restaurant "Maison K" ou on en crée un faux pour la démo
      const { data } = await supabase.from('restaurants').select('id').limit(1).single();
      if (data) setRestaurantUuid(data.id);
    };
    fetchRestaurant();
  }, []);
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      if (!user || !restaurantUuid) throw new Error("Utilisateur non connecté ou restaurant introuvable");
      ...
      // 2. Créer les détails de la commande (Order Items)
      const itemsToInsert = items.map(item => ({
        order_id: newOrder.id,
        product_id: item.id, // Si c'est un UUID, sinon null
        name: item.name,
        price: item.price,
  ...
  if (step === 'success') {
    return (
      <div className="min-h-screen bg-foodiz-black flex flex-col items-center justify-center p-6 text-center animate-fade-in-up">
        ...
        <p className="text-foodiz-gray mb-8">Le restaurant a reçu votre commande. Suivez sa préparation en temps réel.</p>
      ...
      <main className="max-w-lg mx-auto px-4 py-6 space-y-6">
        ...
        <div className="foodiz-card p-6 bg-[#FDFBF7] text-[#1a1a1a]">
          ...
          <div className="space-y-3 font-mono text-sm text-[#5C4033] mb-6">
            ...
            {usePoints && <div className="flex justify-between text-foodiz-green font-bold"><span>Points Foodiz</span><span>-{pointsValue.toFixed(2).replace(".", ",")} €</span></div>}
          </div>
