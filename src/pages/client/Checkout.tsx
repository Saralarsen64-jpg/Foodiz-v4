...
export default function CheckoutPage() {
  ...
  // Mock constants for display (will be replaced by engine in final calc)
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      if (!user || !establishmentId) throw new Error("Utilisateur non connecté ou restaurant introuvable");

      // 1. Préparer les articles pour le moteur économique (prix en centimes)
      const itemsForEngine = items.map(item => ({ 
        partnerPriceCents: Math.round(item.price * 100) 
      }));

      // 2. Lancer le moteur économique Foodiz (2.0 km par défaut)
      const totals = calculateFoodizOrder(itemsForEngine, 2.0);

      // 3. Créer la commande principale avec la répartition financière réelle
      ...
      // 4. Créer les détails de la commande (Order Items)
