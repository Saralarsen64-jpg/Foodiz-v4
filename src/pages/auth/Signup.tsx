  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    const { data, error } = await supabase.auth.signUp({
      ...
      options: {
        ...
        // Redirection après le clic sur le lien email
    ...
    if (error) {
      ...
      setMessage("Un lien de confirmation a été envoyé à votre adresse email. Veuillez vérifier votre boîte de réception (et vos spams).");
    }
  };
  ...
  return (
    <div className="min-h-screen bg-foodiz-black flex items-center justify-center p-4">
      <div className="w-full max-w-md foodiz-card p-8 border border-foodiz-gold/20">
        ...
        {message && (
          <div className={`p-3 rounded-lg text-sm mb-4 ${message.includes("succès") || message.includes("lien") ? "bg-foodiz-green/10 text-foodiz-green" : "bg-foodiz-red/10 text-foodiz-red"}`}>
            {message}
          </div>
        )}
        ...
        <form onSubmit={handleSignup} className="space-y-4">
          ...
          <div className="space-y-2">
            <label className="text-xs text-foodiz-gray uppercase font-bold">Mot de passe</label>
            <div className="flex items-center gap-3 px-4 py-3 rounded-xl border border-foodiz-gold/30 bg-foodiz-black">
