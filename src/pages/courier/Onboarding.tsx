...
import { Bike, Building2, CheckCircle2, FileText, MapPin, Phone, UserRound } from "lucide-react";
...
export default function CourierOnboarding() {
  ...
  return <CourierShell title="Rejoindre la flotte" back="/courier">
    ...
    <section className="foodiz-card mt-4 space-y-4 p-5">{field("name", "Nom complet", UserRound)}{field("phone", "Téléphone", Phone)}{field("legalName", "Nom légal / raison sociale", Building2)}{field("siret", "SIRET (14 chiffres)", FileText)}{field("address", "Adresse professionnelle", MapPin)}{field("postalCode", "Code postal", MapPin)}{field("city", "Ville", MapPin)}<div className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.03] px-4"><Bike size={17} className="text-foodiz-gold"/><select value={form.vehicle} onChange={(event) => setForm((current) => ({ ...current, vehicle: event.target.value }))} className="w-full bg-transparent py-4 text-foodiz-cream outline-none"><option className="bg-foodiz-card" value="bike">Vélo</option><option className="bg-foodiz-card" value="scooter">Scooter</option><option className="bg-foodiz-card" value="motorcycle">Moto</option><option className="bg-foodiz-card" value="car">Voiture</option></select></div><div className="rounded-xl border border-foodiz-gold/15 bg-foodiz-gold/5 p-3 text-xs text-foodiz-gray">Ces informations figureront sur vos justificatifs de règlement. Les coordonnées bancaires seront demandées uniquement via Stripe Connect.</div><button onClick={submit} disabled={sending} className="foodiz-btn flex w-full items-center justify-center gap-2 py-4 disabled:opacity-50">{sending ? <CheckCircle2 size={18}/> : <FileText size={18}/>} {sending ? "Envoi..." : "Envoyer mon dossier"}</button></section>
  </CourierShell>;
}
