...
export default function CourierHelpCenter() {
  ...
  return <CourierShell title="Centre d'aide" back="/courier">
    <section className="rounded-[2rem] border border-weello-gold/20 bg-[linear-gradient(145deg,rgba(216,168,79,0.16),rgba(13,13,13,0.98)_50%)] p-6 text-center">
      <LifeBuoy size={30} className="mx-auto text-weello-gold" />
      <p className="mt-3 text-[10px] font-black uppercase tracking-[.25em] text-weello-gold">Support livreur</p>
      <h2 className="weello-title mt-2 text-2xl">Les bons réflexes Weello</h2>
      ...
      <div className="mt-5 rounded-2xl border border-weello-gold/15 bg-black/25 p-4 text-left">
        <div className="flex gap-3">
          <ShieldCheck size={19} className="shrink-0 text-weello-gold" />
          <p className="text-xs leading-relaxed text-weello-gray">
            Si une situation semble anormale, ne prenez pas de risque : contactez le support avant de forcer une étape.
          </p>
        </div>
      </div>
    </section>
    <div className="mt-4 space-y-3">{topics.map((topic) => <article key={topic.title} className="weello-card flex gap-4 p-5"><div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl border border-weello-gold/20 bg-weello-gold/10 text-weello-gold"><topic.icon size={20} /></div><div><h3 className="font-semibold text-weello-cream">{topic.title}</h3><p className="mt-1 text-xs leading-relaxed text-weello-gray">{topic.text}</p></div></article>)}</div>
    <button onClick={() => navigate("/courier/support")} className="weello-btn mt-4 flex w-full items-center justify-center gap-2 py-4">Analyser un problème<ArrowRight size={17}/></button>
  </CourierShell>;
}
