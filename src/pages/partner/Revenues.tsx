...
export default function PartnerRevenues() {
  ...
  return <div className="min-h-screen bg-weello-black"><header className="sticky top-0 z-30 border-b border-weello-gold/10 bg-weello-card px-4 py-3"><div className="mx-auto flex max-w-6xl items-center gap-3"><button onClick={() => navigate("/partner")} className="text-weello-gold"><ChevronLeft size={20}/></button><h1 className="weello-title text-lg">Revenus & analyses</h1></div></header><main className="mx-auto max-w-6xl space-y-6 px-4 py-6">
    {loading ? <div className="weello-card p-8 text-center text-weello-gray animate-pulse">Calcul des revenus...</div> : <>
      ...
      ].map((item) => <div key={item.label} className="weello-card p-4"><GoldIcon icon={item.icon} size={18}/><p className="mt-3 text-xl font-bold font-serif text-weello-cream">{item.value}</p><p className="mt-1 text-[10px] text-weello-gray">{item.label}</p></div>)}</div>
      <div className="weello-card p-5"><h3 className="weello-title mb-4 text-sm">Six derniers mois</h3><div className="flex h-44 items-end gap-2">{stats.months.map((month) => <div key={month.label} className="flex flex-1 flex-col items-center gap-2"><span className="text-[9px] text-weello-gray">{month.value.toFixed(0)} €</span><div className="w-full rounded-t-lg bg-gradient-to-t from-weello-gold/55 to-weello-gold/20" style={{ height: `${(month.value / maxRevenue) * 100}%` }}/><span className="text-[10px] capitalize text-weello-gray">{month.label}</span></div>)}</div></div>
      <div className="weello-card p-5"><h3 className="weello-title mb-4 text-sm">Produits les plus vendus</h3>{stats.topProducts.length === 0 ? <p className="text-sm text-weello-gray">Aucune vente livrée.</p> : <div className="space-y-3">{stats.topProducts.map((product, index) => <div key={product.name} className="flex items-center justify-between gap-3"><div className="flex items-center gap-3"><span className="w-4 text-[10px] text-weello-gray">{index + 1}</span><span className="text-sm text-weello-cream">{product.name}</span></div><div className="text-right"><span className="text-xs font-semibold text-weello-gold">{product.revenue.toFixed(2)} €</span><p className="text-[9px] text-weello-gray">{product.qty} vendu(s)</p></div></div>)}</div>}</div>
    </>}
  </main></div>;
}
