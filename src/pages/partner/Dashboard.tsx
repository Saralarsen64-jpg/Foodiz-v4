...
import {
  TrendingUp, ShoppingBag, Clock, DollarSign, Bell, Star, Settings, Menu, Wallet, History, Megaphone, LogOut,
} from "lucide-react";
import { supabase } from "../../lib/supabase";
import GoldIcon from "../../components/GoldIcon";
import Logo from "../../components/Logo";

// --- DONNÉES VISUELLES (On garde le design pour l'instant comme demandé) ---
...
export default function PartnerDashboard() {
  ...
  // États pour les VRAIES stats Supabase
    ...
    const fetchData = async () => {
      ...
      if (user) {
        ...
        if (restaurantData) {
            ...
            // --- MOTEUR DE CALCUL RÉEL (Dashboard) ---
            // On récupère les commandes livrées aujourd'hui pour ce restaurant
            const startOfDay = new Date();
            startOfDay.setHours(0, 0, 0, 0);
            
            const { data: orders } = await supabase
                .from('orders')
                .select('partner_total_cents') // On utilise la colonne figée en centimes
            ...
            if (orders) {
                ...
                // Calcul du CA partenaire en euros
                const totalCents = orders.reduce((sum: number, order: any) => sum + (order.partner_total_cents || 0), 0);
                setTodayRevenue((totalCents / 100).toFixed(2).replace(".", ",") + " €");
            }
        }
      }
    };
  ...
  const quickActions = [
    { label: "Foodiz+", icon: Megaphone, path: "/partner/marketing", desc: "Envoyer une campagne" },
    { label: "Historique", icon: History, path: "/partner/orders/history", desc: "Revoir les ventes" },
    { label: "Virements", icon: Wallet, path: "/partner/payouts", desc: "Gérer les gains" },
  ...
  const sidebarItems = [
    ...
    { label: "Commandes", icon: ShoppingBag, path: "/partner/orders/current" },
    ...
    <div className="min-h-screen bg-foodiz-black">
      <header className="bg-foodiz-card border-b border-foodiz-gold/10 px-4 py-3 sticky top-0 z-30">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <button onClick={() => navigate("/partner/orders/current")} className="relative">
            ...
            <span className="absolute -top-1 -right-1 bg-foodiz-gold text-foodiz-black text-[9px] font-bold rounded-full h-4 w-4 flex items-center justify-center">3</span>
          </button>
        </div>
      </header>
      ...
      <main className="max-w-6xl mx-auto px-4 py-6 space-y-8">
        <div className="flex justify-between items-end mb-2">
            ...
            <button onClick={handleLogout} className="hidden md:flex items-center gap-2 text-foodiz-gray hover:text-foodiz-red transition-colors text-sm bg-foodiz-card px-4 py-2 rounded-xl border border-foodiz-gold/10">
                <LogOut size={16} /> Déconnexion
            </button>
        </div>

        <div className="foodiz-card p-6 lg:p-8 border-foodiz-gold/20 bg-[linear-gradient(135deg,rgba(216,168,79,0.12),rgba(17,17,17,0.96)_28%,rgba(5,5,5,1)_100%)] shadow-[0_18px_50px_rgba(0,0,0,0.35)]">
          <p className="text-foodiz-gray text-sm max-w-xl leading-relaxed mb-6">
            Pilotez votre activité, votre carte, vos visuels et vos revenus depuis un cockpit unique.
          </p>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {[
              { label: "Commandes aujourd'hui", value: todayOrders, icon: ShoppingBag, change: "+12 %" },
              { label: "CA du jour", value: todayRevenue, icon: DollarSign, change: "+8 %" },
          ...
          <div className="foodiz-card p-5">
            <div className="flex items-center justify-between mb-5">
              <div>
                ...
                <p className="text-foodiz-gray text-xs mt-1">Évolution du chiffre d’affaires</p>
              </div>
              <div className="flex gap-2">
                {["day", "week", "month", "year"].map((btn) => (
                  <button key={btn} onClick={() => setPeriod(btn as PeriodKey)} className={`px-3 py-1 rounded-full text-[10px] font-medium transition-all ${period === btn ? "bg-foodiz-gold text-foodiz-black" : "bg-foodiz-card border border-foodiz-gold/15 text-foodiz-gray"}`}>
                    {btn === "day" ? "Jour" : btn === "week" ? "Semaine" : btn === "month" ? "Mois" : "Année"}
                  </button>
                ))}
              </div>
            </div>
            <div className="flex items-end gap-2 h-40">
              {chartPoints.map((d) => (
                <div key={d.label} className="flex-1 flex flex-col items-center gap-2">
                  <div className="w-full rounded-t-xl bg-gradient-to-t from-foodiz-gold/55 to-foodiz-gold/18 hover:from-foodiz-gold/70 transition-all" style={{ height: `${(d.value / chartMax) * 100}%` }} />
                  <span className="text-[10px] text-foodiz-gray">{d.label}</span>
                </div>
              ))}
            </div>
            <div className="mt-4 text-center">
              <p className="text-2xl font-serif italic text-foodiz-gold font-bold">{currentRevenue.toFixed(0)} €</p>
              <p className="text-[10px] text-foodiz-gray">CA sur la période (Mock)</p>
            </div>
          </div>
                ...
                <div key={customer.name} className="foodiz-card p-4 flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[9px] px-2 py-0.5 rounded-full bg-foodiz-gold/10 border border-foodiz-gold/15 text-foodiz-gold uppercase">{customer.score}</span>
                    </div>
                    <p className="text-[10px] text-foodiz-gray mt-1">{customer.orders} commandes • {customer.avgBasket.toFixed(2)} €</p>
                  </div>
                </div>
              ))}
            </div>
