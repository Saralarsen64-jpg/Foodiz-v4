...
export default function AdminPayouts() {
  ...
  const exportPayablesCsv = () => {
    ...
    link.href = url;
    link.download = `foodiz-virements-${new Date().toISOString().slice(0, 10)}.csv`;
  ...
  return <AdminShell title="Règlements hebdomadaires" subtitle="Préparation des bordereaux et traçabilité des virements manuels">
    ...
    {loading ? <div className="foodiz-card p-8 text-center text-foodiz-gray animate-pulse">Calcul des montants à payer...</div> : <>
      <section className="grid gap-4 lg:grid-cols-4">
        ...
        <article className="foodiz-card p-5"><Store size={20} className="text-foodiz-gold"/><p className="mt-4 text-[10px] uppercase tracking-widest text-foodiz-gray">Partenaires</p><p className="mt-2 text-3xl font-serif italic text-foodiz-cream">{euros(partnerTotal)}</p><p className="mt-2 text-[10px] text-foodiz-gray">Prix produits à reverser.</p></article>
        <article className="foodiz-card p-5"><Landmark size={20} className="text-foodiz-gold"/><p className="mt-4 text-[10px] uppercase tracking-widest text-foodiz-gray">Livreurs</p><p className="mt-2 text-3xl font-serif italic text-foodiz-cream">{euros(courierTotal)}</p><p className="mt-2 text-[10px] text-foodiz-gray">Livraison + primes - pénalités.</p></article>
        <article className="foodiz-card p-5"><ShieldCheck size={20} className="text-foodiz-gold"/><p className="mt-4 text-[10px] uppercase tracking-widest text-foodiz-gray">Bordereaux</p><p className="mt-2 text-3xl font-serif italic text-foodiz-cream">{draftStatements.length}</p><p className="mt-2 text-[10px] text-foodiz-gray">{paidStatements.length} déjà marqué(s) payé(s).</p></article>
      </section>

      <section className="foodiz-card p-5">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div>
            <p className="text-[10px] uppercase tracking-[0.28em] text-foodiz-gold">Mode coût zéro</p>
            <h2 className="foodiz-title mt-1 text-lg">Préparer les virements manuels</h2>
            <p className="mt-2 max-w-3xl text-xs leading-relaxed text-foodiz-gray">Filtre par ville ou rôle, exporte le CSV, fais les virements depuis ta banque, puis reviens marquer chaque bordereau payé avec la référence bancaire.</p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            {(["all", "partner", "courier"] as const).map((type) => <button key={type} onClick={() => setTypeFilter(type)} className={`rounded-full border px-4 py-2 text-xs ${typeFilter === type ? "border-foodiz-gold bg-foodiz-gold text-black" : "border-white/10 text-foodiz-gray hover:border-foodiz-gold/40"}`}>{type === "all" ? "Tous" : type === "partner" ? "Partenaires" : "Livreurs"}</button>)}
            <select value={cityFilter} onChange={(event) => setCityFilter(event.target.value)} className="rounded-full border border-white/10 bg-black px-4 py-2 text-xs text-foodiz-cream outline-none">
              {cities.map((city) => <option key={city} value={city}>{city === "all" ? "Toutes les villes" : city}</option>)}
            </select>
            <button onClick={exportPayablesCsv} className="rounded-full border border-foodiz-gold/40 px-4 py-2 text-xs text-foodiz-gold hover:bg-foodiz-gold/10"><Download size={14} className="mr-1 inline"/>Exporter CSV</button>
            <button onClick={() => void load()} className="rounded-full border border-white/10 px-4 py-2 text-xs text-foodiz-gray hover:border-foodiz-gold/40"><RefreshCw size={14} className="mr-1 inline"/>Actualiser</button>
          </div>
        </div>
      </section>

      <section><div className="mb-3 flex items-center justify-between"><h2 className="foodiz-title text-lg">À payer</h2><span className="text-[10px] uppercase tracking-widest text-foodiz-gray">{filteredPayables.length} ligne(s)</span></div>{filteredPayables.length === 0 ? <div className="foodiz-card p-5 text-sm text-foodiz-gray">Aucune commande livrée non réglée pour ce filtre.</div> : <div className="grid gap-3 lg:grid-cols-2">{filteredPayables.map((row) => { const key = `${row.beneficiary_type}:${row.beneficiary_id}`; return <article key={key} className="foodiz-card p-5"><div className="flex items-start justify-between gap-3"><div><p className="font-semibold text-foodiz-cream">{row.beneficiary_name}</p><p className="mt-1 text-[10px] uppercase text-foodiz-gray">{row.beneficiary_type === "partner" ? "Partenaire" : "Livreur"} · {row.city || "Ville non renseignée"} · {row.order_count} commande(s)</p>{row.legal_identifier && <p className="mt-1 text-[10px] text-foodiz-gray">SIRET {row.legal_identifier}</p>}</div><p className="text-2xl font-serif italic text-foodiz-gold">{euros(row.amount_cents)}</p></div><p className="mt-4 text-[10px] text-foodiz-gray">Livraisons du {compactDate(row.first_delivery_date)} au {compactDate(row.last_delivery_date)}</p><button disabled={busy === key} onClick={() => void createStatement(row)} className="foodiz-btn mt-4 flex w-full items-center justify-center gap-2 py-3 disabled:opacity-50"><FileText size={16}/>Créer le bordereau</button></article>; })}</div>}</section>
