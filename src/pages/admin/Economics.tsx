    ...
    {loading ? <div className="foodiz-card p-8 text-center text-foodiz-gray animate-pulse">Chargement du registre...</div> : <>
      <section className="foodiz-card overflow-hidden">
        <div className="grid gap-0 lg:grid-cols-[1.15fr_0.85fr]">
          <div className="border-b border-foodiz-gold/10 p-5 lg:border-b-0 lg:border-r">
            <div className="flex items-center gap-3">
              <span className="grid h-11 w-11 place-items-center rounded-2xl border border-foodiz-gold/30 bg-foodiz-gold/10 text-foodiz-gold">
                <Calculator size={20} />
              </span>
              <div>
                <p className="text-[10px] uppercase tracking-[0.28em] text-foodiz-gold">Pilotage gratuit</p>
                <h2 className="foodiz-title text-xl">Marge Foodiz estimée</h2>
              </div>
            </div>
            <p className="mt-4 max-w-2xl text-sm leading-relaxed text-foodiz-gray">
              Cette vue te permet de piloter à moindre coût : Stripe encaisse, Foodiz garde la main sur les virements manuels, et l'écran estime ce qui reste après réserves fidélité/parrainage et frais de paiement simulés.
            </p>
            <div className="mt-5 grid gap-3 md:grid-cols-3">
              <article className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
                <TrendingUp size={18} className="text-foodiz-gold" />
                <p className="mt-3 text-[10px] uppercase tracking-widest text-foodiz-gray">Brut Foodiz</p>
                <p className="mt-1 text-2xl font-serif italic text-foodiz-cream">{euros(foodizOperatingGrossCents)}</p>
                <p className="mt-2 text-[10px] text-foodiz-gray">Foodiz + service + réserve interne.</p>
              </article>
              <article className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
                <Gift size={18} className="text-foodiz-gold" />
                <p className="mt-3 text-[10px] uppercase tracking-widest text-foodiz-gray">Réserves clients</p>
                <p className="mt-1 text-2xl font-serif italic text-foodiz-cream">{euros(growthReserveCents)}</p>
                <p className="mt-2 text-[10px] text-foodiz-gray">Fidélité + parrainage à honorer.</p>
              </article>
              <article className={`rounded-2xl border p-4 ${estimatedNetMarginCents >= 0 ? "border-foodiz-gold/25 bg-foodiz-gold/10" : "border-foodiz-red/30 bg-foodiz-red/10"}`}>
                <Percent size={18} className={estimatedNetMarginCents >= 0 ? "text-foodiz-gold" : "text-foodiz-red"} />
                <p className="mt-3 text-[10px] uppercase tracking-widest text-foodiz-gray">Net estimé</p>
                <p className="mt-1 text-2xl font-serif italic text-foodiz-cream">{euros(estimatedNetMarginCents)}</p>
                <p className="mt-2 text-[10px] text-foodiz-gray">{estimatedNetRate.toFixed(1)}% de l'encaissé client.</p>
              </article>
            </div>
          </div>

          <div className="p-5">
            <p className="text-[10px] uppercase tracking-[0.28em] text-foodiz-gold">Simulation frais Stripe</p>
            <p className="mt-2 text-sm text-foodiz-gray">
            ...
            <div className="mt-5 grid gap-3">
              <label className="rounded-2xl border border-white/10 bg-black/20 p-4">
                <span className="text-[10px] uppercase tracking-widest text-foodiz-gray">Pourcentage estimé</span>
                <div className="mt-2 flex items-center gap-2">
                  <input
                    ...
                    className="w-full bg-transparent text-2xl font-serif italic text-foodiz-cream outline-none"
                  />
                  <span className="text-foodiz-gold">%</span>
                </div>
              </label>
              <label className="rounded-2xl border border-white/10 bg-black/20 p-4">
                <span className="text-[10px] uppercase tracking-widest text-foodiz-gray">Fixe par paiement</span>
                <div className="mt-2 flex items-center gap-2">
                  <input
                    type="number"
                    min="0"
            ...
            <div className="mt-5 rounded-2xl border border-foodiz-gold/20 bg-foodiz-gold/5 p-4 text-xs leading-relaxed text-foodiz-gray">
              <p><span className="text-foodiz-cream">Frais estimés :</span> {euros(estimatedStripeFeesCents)} sur {completedOrderCount} commande{completedOrderCount > 1 ? "s" : ""} payée{completedOrderCount > 1 ? "s" : ""}.</p>
              <p className="mt-2"><span className="text-foodiz-cream">À virer manuellement :</span> {euros(manualPayoutExposureCents)} aux partenaires et livreurs, depuis l'écran Virements.</p>
            </div>
          </div>
        </div>
      </section>
