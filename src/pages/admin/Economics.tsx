...
export default function AdminEconomics() {
  ...
  const accounts = balances ? [
    ...
    { label: "Rémunération livreurs", value: Number(balances.courier_earnings_cents || 0) + Number(balances.courier_prime_cents || 0), icon: HandCoins, detail: "Part article et fonds de prime" },
  ...
  return <AdminShell title="Comptabilité de répartition" subtitle="Journal par commande, réserves et soldes comptables Foodiz">
    ...
    {loading ? <div className="foodiz-card p-8 text-center text-foodiz-gray animate-pulse">Chargement du registre...</div> : <>
      ...
      <section className="foodiz-card overflow-hidden"><div className="border-b border-foodiz-gold/10 p-5"><h2 className="foodiz-title text-lg">Ventilation des commandes</h2><p className="mt-1 text-xs text-foodiz-gray">Les 50 dernières écritures enregistrées.</p></div><div className="overflow-x-auto"><table className="w-full min-w-[1150px] text-left text-xs"><thead className="bg-white/[0.02] text-foodiz-gold"><tr>{["Commande", "Encaissé", "Avantage", "Partenaire", "Livraison", "Livreur", "Foodiz", "Interne + service", "Fidélité", "Parrainage", "Réserve"].map((label) => <th key={label} className="px-4 py-3">{label}</th>)}</tr></thead><tbody className="divide-y divide-white/5">{orders.map((order) => <tr key={order.id}><td className="px-4 py-4"><p className="font-mono text-foodiz-cream">#{order.order_id.slice(0, 8)}</p><p className="mt-1 text-[9px] text-foodiz-gray">{order.restaurant?.name}</p></td><td className="px-4 py-4">{euros(order.client_collected_cents)}</td><td className="px-4 py-4 text-amber-300">{euros(order.advantage_funded_cents)}</td><td className="px-4 py-4">{euros(order.partner_cents)}</td><td className="px-4 py-4">{euros(order.delivery_fee_cents)}</td><td className="px-4 py-4">{euros(Number(order.courier_earnings_cents) + Number(order.courier_prime_cents))}</td><td className="px-4 py-4 text-foodiz-gold">{euros(order.foodiz_revenue_cents)}</td><td className="px-4 py-4">{euros(Number(order.internal_fees_cents) + Number(order.service_fee_cents))}</td><td className="px-4 py-4">{euros(order.loyalty_fund_cents)}</td><td className="px-4 py-4">{euros(order.referral_fund_cents)}</td><td className="px-4 py-4">{euros(order.system_reserve_cents)}</td></tr>)}</tbody></table></div></section>
    </>}
  </AdminShell>;
}
