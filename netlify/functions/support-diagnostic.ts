  ...
  try {
    ...
    if (body.role === "partner") {
      ...
      if (body.category === "delivery") {
        ...
        const { data: delivered } = await adminSupabase.from("orders").select("courier_earnings_cents,courier_prime_fund_cents").eq("courier_id", user.id).eq("status", "delivered");
        const totalCents = (delivered || []).reduce((sum, order) => sum + (order.courier_earnings_cents || 0) + (order.courier_prime_fund_cents || 0), 0);
        const complete = Boolean(bank?.iban && bank?.holder_name);
